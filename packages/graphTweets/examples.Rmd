...
nodes %>% 
  ...
  add_scattergeo(lon = ~longitude, lat = ~latitude,
                 size = ~retweetCount, alpha = 0.5) %>%
  layout(geo = geo, showlegend = FALSE, height=700, width = "100%")
```
...
geo <- list(
  ...
  oceancolor = "#FFFFFF",
  bgcolor = "#FFFFFF",
  caostlinecolor = "#FFFFFF",
  showsubunits = TRUE,
  subunitscolor = "#FFFFFF",
  showcountries = TRUE
)
...
nodes %>% 
  plot_ly(locationmode = 'USA-states', color = I("red"), width = "100%") %>%
  add_scattergeo(lon = ~longitude, lat = ~latitude,
                 size = ~retweetCount, alpha = 0.5) %>%
...
See my other packages and projects at [john-coene.com](http://john-coene.com)
